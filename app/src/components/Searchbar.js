...
class Searchbar extends Component {
	...
	autocomplete(inp) {
		...
		inp.addEventListener("input", function(e) {
			currentFocus = -1;
			searchbarReference.setState({inputValue: this.value})
			searchbarReference.setState({active: true})
		});
	...
	selectAirport(name){
		console.log(this.airports.find( el => el.name === name));
	}
	...
	render() {
		return <form autoComplete="off" action="/action_page.php">
